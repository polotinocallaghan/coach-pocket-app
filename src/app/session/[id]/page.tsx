...
import { dataStore, Session, Exercise, SessionExercise, CalendarEvent } from '@/lib/store';
...
// import { Exercise } from '@/lib/store'; // This line is implicitly removed by the new import
...
export default function SessionDetailPage() {
    ...
    return (
        <div className="min-h-screen bg-slate-900">
            ...
            {/* Ultra-Minimalist "Coach-First" Print Version (Portal) */}
            {typeof document !== 'undefined' && createPortal(
                <div className="print-portal hidden print:block bg-white text-black p-12 w-full h-auto min-h-screen flex flex-col">
                    {/* 1. Header: The Context (Refined Hierarchy) */}
                    <div className="mb-12 border-b-4 border-black pb-6">
                        <h1 className="text-7xl font-black uppercase italic tracking-tighter text-black leading-none mb-2">
                            {session.title} <span className="text-slate-400 font-bold ml-4 not-italic tracking-normal text-5xl">- {totalDuration} MINS</span>
                        </h1>
                        <div className="text-xl font-bold text-slate-400 uppercase tracking-widest mt-2 flex gap-4">
                            <span>{associatedEvent?.date ? new Date(associatedEvent.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }).toUpperCase() : new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }).toUpperCase()}</span>
                            <span>|</span>
                            <span>{exercises.length} BLOCKS</span>
                        </div>
                    </div>

                    {/* 2. The Body: Modular Drill Cards (V3) */}
                    <div className="flex flex-col gap-6 w-full">
                        {exercises.map((item, index) => {
                            ...
                            const duration = item.duration || exercise.duration || 10;

                            // Category Logic (Color Bar Only)
                            ...
                            switch (exercise.category?.toLowerCase()) {
                                ...
                                case 'drill':
                                    ...
                                    break;
                                case 'basket':
                                    barColor = 'bg-purple-500'; // Differentiate basket
                                    break;
                                case 'physical':
                                case 'warmup':
                            ...
                            return (
                                <div
                                    key={`${item.id}-${index}`}
                                    className="relative flex items-center justify-between p-8 bg-slate-50 border border-slate-100 rounded-lg overflow-hidden break-inside-avoid shadow-sm"
                                >
                                    {/* Colored Vertical Bar */}
                                    <div className={cn("absolute left-0 top-0 bottom-0 w-6", barColor)} />

                                    {/* Title (Left) */}
                                    <h3 className="text-4xl font-black uppercase italic tracking-tight text-black ml-8 flex-1 mr-8 break-words hyphens-none">
                                        {exercise.title}
                                    </h3>

                                    {/* Time (Right) */}
                                    <span className="text-3xl font-bold text-slate-500 tracking-wider shrink-0 whitespace-nowrap">
                                        {duration} MIN
                                    </span>
                                </div>
                            );
