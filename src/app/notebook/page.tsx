...
import { dataStore, NotebookNote, Team, TeamMember } from '@/lib/store';
...
export default function NotebookPage() {
    const { user } = useAuth();
    const [notebookMode, setNotebookMode] = useState<'personal' | 'player'>('personal');
    ...
    const targetId = notebookMode === 'personal' ? 'general' : (selectedPlayer?.id || '');

    // Load note for active combination
    useEffect(() => {
        if (notebookMode === 'player' && !selectedPlayer) {
            setNoteContent('');
            return;
        }
    ...
    const handleSaveNote = () => {
        if (notebookMode === 'player' && !selectedPlayer) return;
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-950 text-white pb-20">
            ...
            <div className="sticky top-0 z-50 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 p-4">
                <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        <h1 className="text-xl font-bold">Coach's Notebook</h1>
                    </div>
            ...
            <div className="max-w-7xl mx-auto px-4 mt-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                {/* Center Workspace */}
                <div className={cn(notebookMode === 'personal' ? "lg:col-span-12" : "lg:col-span-9")}>
                    {notebookMode === 'player' && !selectedPlayer ? (
