...
import { dataStore, Exercise, Session, SessionExercise, CalendarSource, CalendarEvent } from '@/lib/store';
import { useDataStore } from '@/lib/useDataStore';
import { cn, generateId } from '@/lib/utils';
import { getRecommendedDrills } from '@/lib/drill-recommender';
import { useSuccess } from '@/lib/SuccessContext';
import { Search, ArrowLeft, Check, Save, Clock, Filter, ChevronDown, Star, X, BookMarked, ChevronLeft, ChevronRight, Target, Users, School, Repeat, Lightbulb, Zap } from 'lucide-react';
...
function SessionBuilderContent() {
    ...
    const [selectedExercises, setSelectedExercises] = useState<{ exerciseId: string; instanceId: string }[]>([]);
    ...
    useEffect(() => {
        ...
        if (sessionIdParam) {
            ...
            if (existingSession) {
                ...
                // Map SessionExercise[] to { exerciseId, instanceId }[]
                // We need to match the structure expected by selectedExercises
                const minimalExercises = existingSession.exercises.map(e => ({
                    exerciseId: e.exerciseId,
                    instanceId: e.id, // Use existing instance ID
                    // We might need to handle duration/notes if we want to preserve them fully in builder
                    // but for now, this just repopulates the list
                }));
    ...
    }, [sessionTitle, sessionDate, sessionTime, sessionType, selectedExercises, sessionSourceId, sessionConfigured]);
    ...
    const toggleExerciseSelection = (exerciseId: string) => {
        ...
        setSelectedExercises([...selectedExercises, { exerciseId, instanceId: generateId() }]);
    };
    ...
    const handleSaveSession = () => {
        ...
        const sessionExercises: SessionExercise[] = selectedExercises.map((item, index) => ({
            ...
            block: 'technical',
            order: index,
            duration: exercises.find(e => e.id === item.exerciseId)?.duration || 10,
        }));
    ...
    const handleSaveAsTemplate = () => {
        ...
        const sessionExercises: SessionExercise[] = selectedExercises.map((item, index) => ({
            ...
            block: 'technical',
            order: index,
            duration: exercises.find(e => e.id === item.exerciseId)?.duration,
        }));
...
function SessionPreviewModal({ sessionTitle, sessionType, sessionDate, selectedExercises, exercises, onClose, onSaveSession, onSaveAsTemplate }: {
    sessionTitle: string;
    sessionType: 'practice' | 'match' | 'match-prep';
    sessionDate: string;
    selectedExercises: { exerciseId: string; instanceId: string }[];
    exercises: Exercise[];
    onClose: () => void;
    onSaveSession: () => void;
