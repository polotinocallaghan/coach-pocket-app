...
import { ArrowLeft, ChevronLeft, ChevronRight, Eye, Trash2, X, CheckCircle, Repeat, Edit, Trophy } from 'lucide-react';
        ...
        <div className="space-y-8">
            ...
            <div>
                                        ...
                                        {dayEvents.slice(0, viewMode === 'month' ? 4 : undefined).map(event => (
                                            ...
                                            >
                                                {viewMode === 'day' && event.time && (
                                                    <span className="font-mono bg-black/20 px-1.5 py-0.5 rounded text-[10px]">{event.time}</span>
                                                )}
                ...
                <div className="mt-6 flex gap-6 justify-center flex-wrap">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="w-3 h-3 rounded-full bg-blue-500"></div>
                        <span className="text-sm text-slate-400">Match Prep</span>
                    </div>
                </div>
            </div>
...
function EventModal({ date, sources, onClose, onSave }: {
    ...
    const router = useRouter();
    const [view, setView] = useState<'type-select' | 'player-select' | 'practice-type'>('type-select');
    ...
    const handlePlayerSelect = (sourceId: string) => {
        ...
        if (selectedType === 'session') {
            ...
            if (source?.type === 'team') {
                ...
                router.push(`/team?date=${dateStr}&teamId=${sourceId}`);
            } else {
                // Redirect to Session Builder (Individual)
                router.push(`/builder?date=${dateStr}&type=practice&sourceId=${sourceId}`);
            }
                            ...
                            {sources.length === 0 ? (
                                ...
                                <div className="grid grid-cols-1 gap-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                    {sources.map((source) => (
                                        <button
                                            key={source.id}
                                            onClick={() => handlePlayerSelect(source.id)}
                                            className="w-full flex items-center gap-3 p-3 rounded-xl border border-slate-700 bg-slate-700/30 hover:bg-slate-700 transition-all text-left"
                                        >
                                            <span className={cn("w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0", source.color)}>
                                                {source.initials}
                                            </span>
                                            <span className="text-white font-medium truncate">{source.name}</span>
                                            {source.type === 'team' && (
                                                <span className="ml-auto text-xs bg-slate-600 text-slate-300 px-2 py-1 rounded">Team</span>
                                            )}
                                        </button>
                                    ))}
                                </div>
...
function ViewEventModal({ event, onClose, onEdit, onDelete, onDeleteSeries, onViewSession, onChartMatch }: {
    ...
    const typeColors: Record<string, string> = {
        ...
        'match-prep': 'bg-blue-500/20 text-blue-400 border-blue-500',
