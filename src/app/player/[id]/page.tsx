...
import {
    ArrowLeft, Trophy, BookOpen, Book,
    ChevronRight, Send, Brain, Shield, Crosshair, MessageSquare, X
} from 'lucide-react';
import { format, isBefore, isAfter, subDays } from 'date-fns';
import { motion, AnimatePresence } from 'framer-motion';

// ── UTR Progression Chart ────────────────────────────────────────────────────
function UTRChart({ events }: { events: CalendarEvent[] }) {
    // Generate synthetic UTR progression from match history
    // Starts at a base and drifts based on wins/losses
    const points = useMemo(() => {
        const sorted = [...events]
            .filter(e => e.type === 'match' && e.completed)
            .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());

        if (sorted.length === 0) {
            // No matches: show pleasant upward mock curve
            return Array.from({ length: 7 }, (_, i) => ({
                label: ['Sep', 'Oct', 'Nov', 'Dec', 'Jan', 'Feb', 'Mar'][i],
                utr: +(9.2 + i * 0.2 + (Math.random() * 0.15 - 0.05)).toFixed(2),
            }));
        }

        let utr = 9.0;
        return sorted.map(m => {
            utr = +(utr + (m.result === 'win' ? 0.15 : -0.08) + (Math.random() * 0.1 - 0.05)).toFixed(2);
            return {
    ...
    const minUTR = Math.min(...points.map(p => p.utr)) - 0.3;
    const maxUTR = Math.max(...points.map(p => p.utr)) + 0.3;
    const range = maxUTR - minUTR || 1;

    const W = 460, H = 110, PADL = 36, PADR = 12, PADT = 12, PADB = 22;
    const innerW = W - PADL - PADR;
    const innerH = H - PADT - PADB;

    const toX = (i: number) => PADL + (i / (points.length - 1)) * innerW;
    const toY = (v: number) => PADT + (1 - (v - minUTR) / range) * innerH;

    const pathD = points
        .map((p, i) => {
            const x = toX(i), y = toY(p.utr);
            if (i === 0) return `M ${x} ${y}`;
            const px = toX(i - 1), py = toY(points[i - 1].utr);
            const cpx = (px + x) / 2;
            return `C ${cpx} ${py}, ${cpx} ${y}, ${x} ${y}`;
        })
        .join(' ');

    const fillD = pathD + ` L ${toX(points.length - 1)} ${H - PADB} L ${toX(0)} ${H - PADB} Z`;

    return (
        <div className="w-full overflow-x-auto">
            <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ minWidth: 280 }}>
                <defs>
                    <linearGradient id="utr-fill" x1="0" y1="0" x2="0" y2="1">
                ...
                {[0, 0.25, 0.5, 0.75, 1].map(t => {
                    ...
                    const val = maxUTR - t * range;
                    return (
                        <g key={t}>
                            <line x1={PADL} y1={y} x2={W - PADR} y2={y} stroke="#1e293b" strokeWidth="1" />
                            <text x={PADL - 4} y={y + 4} textAnchor="end" fill="#475569" fontSize="9">
                                {val.toFixed(1)}
                            </text>
                        </g>
                    );
                ...
                {/* Fill area */}
                <path d={fillD} fill="url(#utr-fill)" />
                ...
                {points.map((p, i) => (
                    <g key={i}>
                        <circle cx={toX(i)} cy={toY(p.utr)} r="5" fill="#0f172a" stroke="#3b82f6" strokeWidth="2.5" />
                        <circle cx={toX(i)} cy={toY(p.utr)} r="2" fill="#3b82f6" />
                    </g>
                ))}

                {/* X-axis labels — show every other label on small datasets */}
                {points.map((p, i) => {
                    if (points.length > 6 && i % 2 !== 0) return null;
                    return (
                        <text key={i} x={toX(i)} y={H - 6} textAnchor="middle" fill="#475569" fontSize="9">
                            {p.label}
                        </text>
                    );
                })}
    ...
    return (
        <div className="flex flex-col h-full bg-slate-800/40 border border-slate-700/50 rounded-2xl overflow-hidden">
            ...
            <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-700/40 bg-slate-800/60">
                ...
                <span className="text-xs font-bold text-white uppercase tracking-wider">Coach's Notebook</span>
            </div>
...
export default function PlayerDashboardPage() {
    ...
    return (
        <div className="pb-20 animate-in fade-in duration-400 space-y-6 max-w-[1300px] mx-auto">
            ...
            {/* ── MIDDLE: Chart + Notebook ────────────────────────────────── */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">

                {/* UTR Progress — compact */}
                <div className="bg-slate-800/40 border border-slate-700/50 rounded-2xl p-4">
                    <div className="flex items-center justify-between mb-3">
                        <h2 className="text-xs font-bold text-white uppercase tracking-wider">UTR Progression</h2>
                        <span className="text-[10px] text-slate-600 bg-slate-800 border border-slate-700 px-2 py-0.5 rounded-lg">Last 6 Months</span>
                    </div>
                    <UTRChart events={playerEvents} />
                </div>
