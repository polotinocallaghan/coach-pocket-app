...
export interface Session {
    ...
    rating?: number; // 1-5
    feedback?: string; // Analysis/Notes
    completedAt?: Date;
}
...
class DataStore {
    ...
    public getEffectiveRole(): 'coach' | 'player' {
        // Hard-enforce: A real player can never view the app as a coach
        if (this.userProfile?.role?.toLowerCase() === 'player') {
            return 'player';
        }

        // Demo override only applies to coaches
        if (typeof window !== 'undefined') {
            const demoRole = localStorage.getItem('demo_role');
            if (demoRole === 'player' || demoRole === 'coach') {
                return demoRole;
            }
        }
        ...
        if (typeof window !== 'undefined') {
            try {
                ...
                if (exercises || sessions || calendar || playlists || teams || profile) {
                    this.exercises = exercises ? JSON.parse(exercises).map((e: any) => ({
                        ...e,
                        playMode: e.playMode || 'both', // backfill for cached data
                    })) : [];
                    this.sessions = sessions ? JSON.parse(sessions).map((s: any) => ({
                        ...s,
                        createdAt: new Date(s.createdAt)
                    })) : [];
                    this.calendar = calendar ? JSON.parse(calendar).map((e: any) => ({
                        ...e,
                        date: new Date(e.date)
                    })) : [];
                    this.playlists = playlists ? JSON.parse(playlists) : [];
                    this.teams = teams ? JSON.parse(teams) : [];
                    this.programs = programs ? JSON.parse(programs).map((p: any) => ({
                        ...p,
                        startDate: new Date(p.startDate),
                        createdAt: new Date(p.createdAt),
                    })) : [];
                    this.notifications = notifications ? JSON.parse(notifications).map((n: any) => ({
                        ...n,
                        createdAt: new Date(n.createdAt)
                    })) : [];
                    this.notebookNotes = notebookNotes ? JSON.parse(notebookNotes).map((n: any) => ({
                        ...n,
                        createdAt: new Date(n.createdAt),
