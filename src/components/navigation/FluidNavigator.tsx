...
export function FluidNavigator({ children }: FluidNavigatorProps) {
    ...
    }, [user, pathname]); // Re-check on nav changes in case profile updates

    // Filter Items
    const visibleNavItems = navItems.filter(item => {
        if (role === 'player') {
            // Players only see Home (Dashboard) and Calendar
            return ['/', '/calendar'].includes(item.href);
        }
        return true; // Coaches see everything
    });
    ...
    return (
        <div className="min-h-screen flex flex-col relative">
            ...
            <Suspense fallback={<div className="h-20 bg-slate-800/50 sticky top-0 z-50 backdrop-blur-xl border-b border-slate-700/50" />}>
                <FluidHeader visibleNavItems={visibleNavItems} activeIndex={activeIndex} role={role} />
            </Suspense>
                        ...
                        <div className="flex items-center gap-4 z-10">
                            ...
                            <Link href="/" className="flex items-center gap-3 hover:opacity-80 transition-opacity">
                                ...
                                <div className="hidden sm:block">
                                    ...
                                    <p className="text-xs text-slate-400 font-medium">{role === 'player' ? 'Player Portal' : 'Pro Edition'}</p>
                                </div>
                            </Link>
                        </div>
