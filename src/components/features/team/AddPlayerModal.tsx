...
import { dataStore, Team } from '@/lib/store';
...
export default function AddPlayerModal({ isOpen, onClose, onPlayerAdded }: AddPlayerModalProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        // Must have a selected team
        if (!selectedTeamId) {
            alert("A player must be assigned to an existing team (or you can create an 'Individual Players' team explicitly).");
            return;
        }
                        ...
                        <div>
                            ...
                            {teams.length > 0 ? (
                                ...
                                <div className="text-xs text-slate-500 bg-slate-800 p-3 rounded-lg border border-slate-700 flex items-start gap-2">
                                    ...
                                    No teams available. You can assign them to a team later.
                                </div>
                            )}
                        </div>
                    </div>
