...
import { dataStore, CalendarSource, Team, CalendarEvent, SessionExercise } from '@/lib/store';
...
export default function ProfileDashboard({ isOpen, onClose }: ProfileDashboardProps) {
    ...
    const isPlayer = realProfile?.role?.toLowerCase() === 'player';
            ...
            >
                ...
                <div className="flex w-full h-full">
                    ...
                    <div className="w-80 bg-slate-800/50 border-r border-slate-700 flex flex-col p-6">
                            ...
                            {!isPlayer && (
                                <>
                                    ...
                                    >
                                        ...
                                        Players
                                        <span className="ml-auto bg-slate-800 px-2 py-0.5 rounded-md text-xs">{players.length}</span>
                                    </button>
                                    <button
                                        onClick={() => { setActiveTab('teams'); setSelectedPlayerId(null); }}
                                        className={cn(
                                            "w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium text-sm",
                                            activeTab === 'teams'
                                                ? "bg-green-500/10 text-green-400 border border-green-500/20"
                                    ...
                                    >
                                        ...
                                        Teams
                                        <span className="ml-auto bg-slate-800 px-2 py-0.5 rounded-md text-xs">{teams.length}</span>
                                    </button>
                                    <div className="w-full h-px bg-slate-700/50 my-2" />
                                    <button
                                        onClick={() => { router.push('/notebook'); onClose(); }}
                                        className="w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium text-sm text-slate-400 hover:bg-slate-800 hover:text-white group"
                                    >
                                        <Book className="w-4 h-4 text-green-400 group-hover:text-green-300" />
                                        Coach Notebook
                                    </button>
                                </>
                            )}
                        ...
                        {/* Demo Toggle for Developer/Coach */}
                        {!isPlayer && (
                            <div className="mt-8 pt-6 border-t border-slate-700 mb-4">
                                <p className="text-xs text-slate-500 mb-3 uppercase tracking-wider font-bold">App View Mode</p>
                                <div className="flex bg-slate-800 rounded-lg p-1 border border-slate-700">
                                    <button
                                        onClick={() => { localStorage.setItem('demo_role', 'coach'); window.location.reload(); }}
                                        className={cn("flex-1 py-1.5 text-xs font-medium rounded-md transition", dataStore.getEffectiveRole() === 'coach' ? "bg-slate-700 text-white shadow-sm" : "text-slate-400 hover:text-white")}
                                    >
                                        Coach
                                    </button>
                                    <button
                                        onClick={() => { localStorage.setItem('demo_role', 'player'); window.location.reload(); }}
                                        className={cn("flex-1 py-1.5 text-xs font-medium rounded-md transition", dataStore.getEffectiveRole() === 'player' ? "bg-green-600 text-white shadow-sm" : "text-slate-400 hover:text-white")}
                                    >
                                        Player
                                    </button>
                                </div>
